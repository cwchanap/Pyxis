...
- 6-point horizontal outer inset and 4-point vertical outer inset;
...
- header, wrapping trait, and matchup/lane rows in the remaining content
  column; and
- minimum label size of 9 points.
...
The country-complete state centers `Country <number> conquered` in the same
panel and creates no active Attack node or Attack hit frame.
...
4. If the existing current-city control contains the point, request entry for
   the current city.
5. If a city interaction frame contains the point, classify it with
   `mapStatus(for:)`:
...
If the router is absent, the locally entered state is not saved, routing stays
enabled, and `Cannot enter city yet.` is shown. If a race changes the entry
result, `.locked` shows `City N is locked`, `.alreadyCompleted` shows
...
- match `KingdomGameState.goldReward(for:)`.
