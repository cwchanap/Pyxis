...
3. **`BattleCombatState`** (`Pyxis/BattleCombatState.swift`) — pure live-combat simulator. Owns soldier roster (each `Soldier` has a `SoldierType` and is spawned `manual` or from a `building`) + tower cooldown. Each `tick(deltaTime:cityRemainingHP:)` advances soldier positions, resolves tower shots, applies soldier attacks against the city, prunes dead soldiers, and returns a `TickResult` (`cityDamage`, conquest flag, typed `soldierAttacks` / `soldierLosses`, tower shots, damaged soldier IDs). Soldier HP and range vary by type. Each soldier is randomly assigned one of three `BattleLane`s (`left`/`center`/`right`) at spawn via a seedable internal PRNG (`SplitMix64`); the tower targets a random occupied lane per shot and scales its damage by `Configuration.laneDamageMultipliers`. `Country1CityCatalog` owns each authored city's `LaneDefenseProfile`: one fortified lane (1.25× tower damage), one exposed lane (0.80×), and one standard lane. The catalog is also the single authored source of Country 1 city identity — `name`, `flavorText`, `conquestTitle`, and the `displayTitle` computed on `CityDefinition`; this identity is compiled catalog content, never persisted in `KingdomGameState` or `BattleResult` (a `BattleResult` carries only its `cityKey`). Conquest reports resolve their title from `BattleResult.cityKey` via the static `KingdomGameState.displayConquestTitle(for:)` (no ambient state), and `ConquestReportContent.project(from:title:)` takes that caller-owned title. `KingdomGameState.currentCityLaneDefenseProfile` projects the active city's catalog definition, and `BattleScene` feeds its multipliers into the combat configuration. `BattleScene` drives combat and records typed attack/loss events into the active siege session via `KingdomGameState.applyLiveSoldierAttacks(_:)` / `recordSoldierLosses(_:)`.
...
5. **Scenes** — code-owned SpriteKit scenes (the bundled `GameScene.sks` / `Actions.sks` are unused). Each holds its own copy of `KingdomGameState`, persists after every mutation, and rebuilds layout on `didChangeSize`. Shared UI helpers live in `GameUIComponents.swift` (`PanelNode`, `ProgressBarNode`) and `GameUITheme.swift` (fonts, colors, Z-order, safe-area insets).
   - `BattleScene` (`Pyxis/BattleScene.swift`) — owns a `BattleCombatState`, spawns/upgrades soldiers, runs the per-frame tick (which also calls `resolveActiveBuildingSpawns` to inject building-produced soldiers), mirrors `TickResult` into UI (HP bar, soldier nodes) on a vertical full-screen battlefield (enemy city top, player castle bottom, three marching lanes), and routes to the map or the building view via `BattleSceneRouting`. On conquest it renders the `pendingBattleResult` as a compact report via `ConquestReportNode` (`Pyxis/ConquestReportNode.swift`) driven by `ConquestReportLayout` (`Pyxis/ConquestReportLayout.swift`) / `ConquestReportContent` (`Pyxis/ConquestReportContent.swift`); the report origin is gated so fresh effects (gold burst, city-conquest feedback) play exactly once and never replay on restoration or resize. Continue acknowledges and saves (`acknowledgePendingBattleResult` + `store.save`) before routing, and disables itself so it can't fire twice. Battlefield geometry is computed by `BattlefieldLayout` (`Pyxis/BattlefieldLayout.swift`) — a pure-value (`CoreGraphics`-only, no SpriteKit/UIKit) `Equatable` struct whose `compute(constraints:)` maps scene dimensions to per-lane castle/enemy gate points and structure sizes (with a collapsed-gate fallback when the area is too small); `BattleScene` reads those values and applies them to its nodes. It is unit-tested in isolation (`PyxisTests/BattlefieldLayoutTests.swift`).
   ...
   - `BuildingViewScene` (`Pyxis/BuildingViewScene.swift`) — renders the 25-slot grid for the current city, builds/upgrades buildings via `buildBuilding`/`upgradeBuilding` (showing affordability/cap feedback), and routes back to battle via `BuildingViewSceneRouting`. A build/upgrade settles pending building production first, which can itself conquer the city mid-action. Conquest that happens while the player was not looking (idle foreground/gate-resume settlement, plus Map current-city RETURN settlement) routes to Battle so the pending report is shown. Conquest caused by a deliberate in-place Building View build/upgrade action stays on Camp, leaves the pending result in place, and shows a short pointer to open Battle for the report.
   - `RecommendedCampRecommendation` (`Pyxis/RecommendedCampRecommendation.swift`) is a pure Building View assistance projection: deterministic, non-persisted, and intentionally not an optimizer. Authored favorable order plus the existing building-state, unlock, cap, and cost APIs are authoritative. The current policy strengthens an existing favored building before another copy; this is a simple heuristic, not a damage/gold guarantee. Building View recomputes before explicit Ready execution and delegates to existing mutation methods. The row is packed inside the existing action-panel height, so scenic-grid space is unchanged. Do not add a recommendation platform or service without a concrete second consumer. HPA-365 does not add broader Building View accessibility; the existing palette/lot controls have no dedicated accessibility surface.
...
- Game rules belong in the pure models (`KingdomGameState`, `BattleCombatState`, `CityBuildingState`/`SoldierType`) — none import SpriteKit/UIKit. Keep these models framework-free so they stay unit-testable. The same discipline applies to layout/geometry math: extract it into a pure value type (as `BattlefieldLayout` does, importing only `CoreGraphics`) so the scene only applies precomputed values and the math can be tested without a running scene.
- Gameplay feedback is observational and restrained; it must not change combat, persistence, routing, or economy.
- On the Country Map, the `.flavor` and `.idleSummary` Scout-card transients are the non-blocking map-feedback kinds (`CountryMapTransientFeedback.Kind.blocksScoutEntry == false`); they overlay via `nonBlockingOverlayFrame` (which excludes `attackFrame`), so flavor and idle summaries never disable the Attack action — every other map-feedback kind blocks both Attack and scout entry.
...
- Sound output is asynchronous and drop-only: before readiness, after an activation failure, or while disabled, sounds are dropped rather than queued or replayed. The mixer has exactly eight voices (automatic `0...5`, protected/manual `6...7`), protects manual/outcome cues from automatic eviction, and stops/deactivates on background/interruption. Recovery only restores readiness; it does not reactivate ambience or replay cues. Haptics follow the same semantic policy and preference gate without affecting gameplay state.
- `GameViewController` is the one production composition root for the shared feedback preferences, clock, AV backend, sound controller, haptic output, and coordinator. It binds the accessibility adapter only after its `SKView` exists, injects the same runtime into Battle, Map, and Building scenes, prepares sound asynchronously without delaying first render, and removes its lifecycle observers on teardown. A layout gate must preserve an open settings modal without synthesizing catch-up feedback.
- The Settings sheet exposes only the persisted Sound Effects and Haptics preferences. Its controls must remain keyboard/VoiceOver accessible, use converted SpriteKit frames for accessibility geometry, preserve the visible state across scene replacement, and never own playback, persistence side effects beyond the preference write, or gameplay mutations.
