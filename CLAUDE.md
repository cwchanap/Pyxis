...
- HPA-389 projects each `BattleCombatState.TickResult` into exactly one ordered automatic-combat batch; the policy validates automatic sources, coalesces and rate-limits them, and maintains fairness so a third successful non-attack selection reserves an eligible attack (with siege/ranged/melee rotation). Fresh conquest reward precedes outcome feedback. Feedback is observational only: it must not change combat, persistence, routing, or economy; restoration, resize, background catch-up, and automatic building-spawn damage do not replay stale effects.
